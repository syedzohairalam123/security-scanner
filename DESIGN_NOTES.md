...
## Response identity and repeat-scan integrity

HTTP headers can describe an interstitial rather than the page a person meant
to scan. `scanner/scan_integrity.py` therefore records a lightweight baseline
on each completed scan: the page title, a content-length bucket, a SHA-256
hash of normalized visible text, and the set of response header names. The
next scan of the same target compares its fingerprint with the most recent
completed scan. A sufficiently large unexplained delta is reported as a
medium-confidence `scan_integrity` finding, while a direct known interstitial
phrase remains a high-confidence finding.

When the first homepage response trips the integrity check, the orchestrator
waits two seconds and performs exactly one ordinary GET retry. It never
injects cookies, renders JavaScript, changes identity, or tries to defeat a
challenge. If the retry is clean, all other checks use that response and the
finding records what happened; if it is still an interstitial, the original
finding is retained and no further requests are made for this purpose. The
session also sends standard `Accept`, `Accept-Language`, and `Accept-Encoding`
headers while keeping the honest Perimeter-Scanner User-Agent.
